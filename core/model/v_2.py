...
class ExtremumData(BaseModel):
    extr_all_index: Slice = Field(default=Slice(begin=0, end=0))
    extr_min_index: Slice = Field(default=Slice(begin=0, end=0))
    extr_max_index: Slice = Field(default=Slice(begin=0, end=0))
...
class CombinedExtremum:
    ...
    def __init__(self, values: np.ndarray[float], split: int, batch: int = None):
        ...
        for i in range(0, self._len_values, self._batch):
            self._ptr_extr[i // self._batch] = ExtremumData(
                extr_all_index=Slice(begin=i, end=min(i + self._batch, self._len_values))
            )
    ...
    def localization_extremes(self, coincident: int = 1, eps: int = 1) -> "CombinedExtremum":
        ...
        for sub_interval, data in self._ptr_extr.items():
            begin = data.extr_all_index.begin
            end = data.extr_all_index.end
            ...
            extr_all_index.extend(_extr_all_index)
            extr_min_index.extend(_extr_min_index)
            extr_max_index.extend(_extr_max_index)

            _data = ExtremumData(
                extr_all_index=Slice(begin=offset_all, end=offset_all + len(_extr_all_index)),
                extr_min_index=Slice(begin=offset_min, end=offset_min + len(_extr_min_index)),
                extr_max_index=Slice(begin=offset_max, end=offset_max + len(_extr_max_index)),
        ...
        extr_min_index = np.array(extr_min_index)
        extr_max_index = np.array(extr_max_index)

        print(self._indexes[extr_all_index])
        print(self._indexes[extr_min_index])
        print(self._indexes[extr_max_index])

        self._values = self._values[extr_all_index]
        self._indexes = self._indexes[extr_all_index]
    ...
    def _filter_extremes(self, _offset: int, _eps_min: int, _eps_max: int):
        ...
        for i in range(_n):
            ...
            if is_min_extr:
                ...
                if is_add_index:
                    _extremes[_k_all] = i + _offset
                    _extremes_min[_k_min] = i + _offset
                    _k_all += 1
                    _k_min += 1
                    is_added = True
            ...
            if is_max_extr and not is_added:
                ...
                if is_add_index:
                    _extremes[_k_all] = i + _offset
                    _extremes_max[_k_max] = i + _offset
                    _k_all += 1
                    _k_max += 1
...
def main():
    ...
    trend.localization_extremes(coincident=coincident, eps=eps).localization_extremes(coincident=coincident, eps=eps).localization_extremes(coincident=coincident, eps=eps).localization_extremes(coincident=coincident, eps=eps)
